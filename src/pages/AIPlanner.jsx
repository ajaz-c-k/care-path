...
const AIPlanner = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch hospitals and treatments
                ...
                // Flatten/Map to match the UI expectations if needed
                const mappedHospitals = (hospitalsData || []).map(h => ({
                    ...h,
                    // Take the first treatment as the representative one for Step 1
                    representativeTreatment: h.treatments?.[0] || { 
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            if (searchData.location) {
                query = query.or(`city.ilike.%${searchData.location}%,country.ilike.%${searchData.location}%`);
            }

            const { data: hospitalsData, error: hospitalsError } = await query;
            
            if (hospitalsError) throw hospitalsError;

            let mappedHospitals = (hospitalsData || []).map(h => ({
                ...h,
                representativeTreatment: h.treatments?.find(t => 
                    t.treatment_name.toLowerCase().includes(searchData.treatment.toLowerCase())
                ) || h.treatments?.[0] || { 
                    treatment_name: 'General Treatment', 
                    cost_inr: 200000, 
                    doctor_specialization: 'General' 
            ...
            // Client-side filtering for treatment name and budget if needed
            if (searchData.treatment) {
                mappedHospitals = mappedHospitals.filter(h => 
                    h.treatments?.some(t => t.treatment_name.toLowerCase().includes(searchData.treatment.toLowerCase())) ||
                    h.name.toLowerCase().includes(searchData.treatment.toLowerCase())
                );
            }

            setHospitals(mappedHospitals);
        } catch (error) {
            console.error('Error searching hospitals:', error.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const costBreakdown = useMemo(() => {
        ...
        const baseCost = selectedHospital.representativeTreatment.cost_inr;
        ...
        const total = baseCost + flight + hotel + medicine + transport;
        
        return { surgery: baseCost, flight, hotel, medicine, transport, total };
    }, [selectedHospital, travelData]);

    const cheapestPlan = useMemo(() => {
        if (hospitals.length === 0) return { hospital: 'N/A', total: 0 };
        const cheapestHospital = hospitals.reduce((prev, curr) => 
            prev.representativeTreatment.cost_inr < curr.representativeTreatment.cost_inr ? prev : curr
        );
        const flight = 35000 * travelData.travelers;
        const hotelMultipliers = { 'Budget': 0.6, 'Standard': 1, 'Premium': 1.8 };
        const hotel = 18000 * hotelMultipliers[travelData.hotelClass];
        const cost = cheapestHospital.representativeTreatment.cost_inr + flight + hotel + 8000 + 5000;
        return { hospital: cheapestHospital.name, total: cost };
    }, [hospitals, travelData]);
        ...
        <div className="max-w-6xl mx-auto px-4 py-12">
            ...
            {loading ? (
                <div className="py-20 flex flex-col items-center gap-4">
                    ...
                    <p className="text-text-muted text-lg font-medium">Analyzing medical data...</p>
                </div>
            ) : (
                <>
                            ...
                            <div className="grid md:grid-cols-3 gap-8">
                                {hospitals.length > 0 ? hospitals.map(h => (
                                    <div key={h.id} className="card hover:border-primary/30 transition-all hover:-translate-y-1 duration-300 group">
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            <div className="flex items-center gap-1 bg-yellow-50 text-yellow-600 px-2 py-1 rounded-lg text-sm font-bold">
                                                <Star size={14} fill="currentColor" /> {h.rating || 'N/A'}
                                            </div>
                                        </div>
                                        <p className="text-sm text-text-muted mb-4 font-medium">{h.representativeTreatment.doctor_specialization}</p>
                                        <div className="mt-auto pt-4 border-t border-border-color flex justify-between items-center">
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-primary">₹{(h.representativeTreatment.cost_inr/100000).toFixed(1)}L</p>
                                            </div>
                                            <button 
                                    ...
                                    <div className="md:col-span-3 text-center py-12">
                                        <p className="text-text-muted">No hospitals found matching your criteria.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                                ...
                                <div className="p-8 space-y-8">
                                    <div className="grid grid-cols-2 gap-8">
                                        ...
                                        <div className="flex gap-4 items-start">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-lg font-bold text-text-main">{selectedHospital.representativeTreatment.stay_duration_days} days</p>
                                            </div>
                                        </div>
                                        <div className="flex gap-4 items-start">
                                    ...
                                    <div className="p-4 bg-gray-50 rounded-xl border border-border-color">
                                        <p className="text-sm text-text-main leading-relaxed">
                                            <strong>AI Insight:</strong> Based on your interest in {selectedHospital.representativeTreatment.treatment_name}, {selectedHospital.name} offers a {selectedHospital.success_rate}% success rate. Expert {selectedHospital.representativeTreatment.doctor_specialization} care is available for your procedure.
                                        </p>
                                    </div>
            ...
            <div className="mt-20">
                ...
                <HospitalMap markers={hospitals.map(h => ({
                    id: h.id,
                    name: h.name,
                    treatment: h.representativeTreatment.treatment_name,
                    cost: `₹${(h.representativeTreatment.cost_inr/100000).toFixed(1)}L`,
                    rating: h.rating,
                    position: [9.9312 + (Math.random() - 0.5) * 0.1, 76.2673 + (Math.random() - 0.5) * 0.1] // Mock positions if missing
                }))} />
            </div>
        </div>
    );
